...
const projects = JSON.parse(readFileSync("app/data/projects.json", "utf8"));
const publicUrls = [
  `${siteOrigin}${basePath}/`,
  `${siteOrigin}${basePath}/projects/`,
  ...projects.map((project) => `${siteOrigin}${basePath}/projects/${project.routeSlug}/`),
];
...
if (publicUrls.length !== 2 + projects.length) throw new Error("Static route list does not match the project catalog");
