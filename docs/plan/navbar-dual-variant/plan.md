...
> Historical design plan, superseded by the React Router migration. Its Astro paths describe the former runtime and are not current implementation guidance.
>
> **Original goal:** Refactor inline navbar in `MainLayout.astro` into two parallel components, each on its own branch. Direction A preserves the current Command Center look; Direction B adopts iOS 17/18 compact tab bar style. Both share mobile hamburger overlay + accessibility baseline.
