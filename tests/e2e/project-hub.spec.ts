...
test("populated Hub orders, filters and resolves project selection", async ({ page }) => {
  ...
  await expect(page.locator("[data-project-title]")).toHaveText(["Data First", "Software First", "Software Later"]);
  await expect(panel).toContainText("Overview of the first data engineering project.");
  await expect(panel.locator("[data-project-status]")).toHaveText("Status: Completed");
  await expect(page.locator('[data-project-select="data-first"]')).toHaveAccessibleName("Data First Status: Completed");
  ...
  await expect(panel.locator("[data-project-status]")).toHaveText("Status: Building");
