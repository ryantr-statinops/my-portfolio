...
import { CATEGORY_MAP, SITE } from "../../src/lib/constants";
  ...
  return (
    <article className="mx-auto max-w-4xl px-6 py-32 md:px-8">
      <header className="mb-16 space-y-6 border-b border-border pb-16">
        <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-widest text-primary"><span className="rounded border border-primary/20 px-2 py-1">{CATEGORY_MAP[project.category]}</span><span className="text-muted">•</span><time className="font-mono text-muted" dateTime={project.date}>{project.date}</time></div>
        ...
        <div className="space-y-4 pt-4">
          <div className="flex flex-wrap gap-2"><span className="mb-1 w-full text-[10px] font-bold uppercase tracking-[0.2em] text-muted">Stack</span>{project.stack.map((technology) => <span key={technology} className="rounded border border-primary/10 bg-primary/5 px-2 py-0.5 text-[10px] font-bold uppercase tracking-tight text-primary">{technology}</span>)}</div>
          <div className="flex flex-wrap gap-2"><span className="mb-1 w-full text-[10px] font-bold uppercase tracking-[0.2em] text-muted">Focus Areas</span>{project.tags.map((tag) => <span key={tag} className="rounded-full border border-border/50 bg-muted/5 px-3 py-1 text-xs font-medium text-muted">{tag}</span>)}</div>
          <p className="border-l-2 border-primary pl-4 text-sm text-muted">{project.impact}</p>
          <div className="flex flex-wrap gap-3">{project.links.github && <a className="rounded border border-border px-4 py-2 text-sm hover:border-primary" href={project.links.github} target="_blank" rel="noreferrer">Source Repository</a>}{project.links.demo && <a className="rounded border border-border px-4 py-2 text-sm hover:border-primary" href={project.links.demo} target="_blank" rel="noreferrer">Live Demo</a>}</div>
        </div>
      </header>
